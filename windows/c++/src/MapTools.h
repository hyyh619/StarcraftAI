...
class MapTools
{
    ...
    Grid<int> m_groundScouterWalk;  // the ground scouter has walked through the path.
...
public:
    ...
    // Add by hy
    BWAPI::TilePosition GetTheFarestCreepAgainstEnemy(BWAPI::UnitType type, BWAPI::TilePosition center);
    BWAPI::TilePosition GetNearestByPos(BWAPI::UnitType type, BWAPI::TilePosition center, bool buildingOnCreep);
    int GetValueOfFirstScouter(BWAPI::TilePosition pos);
    void GetBoundingBoxFromCenter(BoundingBox& box, BWAPI::TilePosition center, int size);
};