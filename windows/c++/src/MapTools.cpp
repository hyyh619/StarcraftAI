...
void MapTools::onStart()
{
    ...
    m_groundScouterWalk = Grid<int>(m_width, m_height, 0);
...
{
    Unit *pFirstGroundUnit = m_pBot->GetFirstGroundScouter();

    if (pFirstGroundUnit)
    {
        BWAPI::Position     pos = pFirstGroundUnit->m_unit->getPosition();
        BWAPI::TilePosition tile(pos);

        m_groundScouterWalk.set(tile.x, tile.y, g_nFrames);
    }
...
BWAPI::TilePosition MapTools::GetNearestByPos(BWAPI::UnitType type, BWAPI::TilePosition center, bool buildingOnCreep)
{
    BWAPI::TilePosition buildPos;
    int                 maxBuildRange = 2;

    buildPos = BWAPI::Broodwar->getBuildLocation(type, center, maxBuildRange, buildingOnCreep);
    if (!isValidTile(buildPos.x, buildPos.y))
    {
        maxBuildRange *= 2;
        buildPos       = BWAPI::Broodwar->getBuildLocation(type, center, maxBuildRange, buildingOnCreep);
        if (!isValidTile(buildPos.x, buildPos.y))
...
BWAPI::TilePosition MapTools::GetTheFarestCreepAgainstEnemy(BWAPI::UnitType type, BWAPI::TilePosition center)
{
    BoundingBox         box;
    BWAPI::TilePosition buildPos;
    BWAPI::TilePosition pos              = center;
    int                 searchRange      = 8;
    int                 mostValueInCreep = 0;
    BWAPI::TilePosition nearestByEnemy(pos);
    int                 maxBuildRange = 4;

    // Search from start position  8x8
    // Bounding box
    while (1)
    {
        GetBoundingBoxFromCenter(box, pos, searchRange);

        // Search bounding box for the start point of first groud scouter.
        for (int x = box.left; x <= box.right; x++)
        {
            for (int y = box.top; y <= box.bottom; y++)
            {
                BWAPI::TilePosition search(x, y);
                int                 value = GetValueOfFirstScouter(search);

                if (value > mostValueInCreep &&
                    BWAPI::Broodwar->hasCreep(search) &&
                    BWAPI::Broodwar->canBuildHere(search, type))
                {
                    mostValueInCreep = value;
                    nearestByEnemy   = search;
                }
            }
        }

        // There is no scouter.
        if (mostValueInCreep > 0)
            break;

        searchRange *= 2;

        // Can not find the correct place to build.
        if (searchRange > 32)
            break;
    }
    ...
    if (mostValueInCreep == 0)
    {
        ...
        maxBuildRange = 32;
        buildPos      = BWAPI::Broodwar->getBuildLocation(type, nearestByEnemy, maxBuildRange, true);
        return buildPos;
    }

    // Start from the start point of first ground scouter.
    while (1)
    {
        bool bFound = false;

        GetBoundingBoxFromCenter(box, nearestByEnemy, 2);
...
int MapTools::GetValueOfFirstScouter(BWAPI::TilePosition pos)
{
    return m_groundScouterWalk.get(pos.x, pos.y);
}

void MapTools::GetBoundingBoxFromCenter(BoundingBox &box, BWAPI::TilePosition center, int size)
{
    // Bounding box
    box.left   = center.x - size;
    box.right  = center.x + size;
    box.top    = center.y - size;
    box.bottom = center.y + size;

    // Check bounding box exceeds the map.
    if (box.left <= 0)
        box.left = 1;
    ...
    if (box.right >= m_width)
        box.right = m_width - 1;

    if (box.bottom >= m_height)
        box.bottom = m_height - 1;
}